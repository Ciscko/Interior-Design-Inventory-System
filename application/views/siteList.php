        ...
        <a class="btn btn-primary" href="<?php echo base_url('export/export_table/sitelogs');?>" ><i class="glyphicon glyphicon-download-alt"></i> Excel Export</a>
        <br />
        <br />
        <table id="table" class="table table-striped table-bordered" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <th>Site End Date</th>
                    <th>Site Invoice NO</th>
                    <th>Site Invoiced Amount</th>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('site/ajax_list')?>",
            "type": "POST"
        },
...
function edit_site(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('site/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        ...
        url = "<?php echo site_url('site/ajax_update')?>";
    }
...
function delete_site(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('site/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "<?php echo site_url('site/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="id" placeholder="Site ID" class="form-control" type="text">
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Site Start Date</label>
                            <div class="col-md-9">
                                <input name="start" placeholder="" class="form-control datepicker" type="text">
                                <span class="help-block"></span>
                            </div>
                        </div>

                        <div class="form-group">
                            <label class="control-label col-md-3">Site End Date</label>
                            <div class="col-md-9">
                                <input name="end" placeholder="" class="form-control datepicker" type="text">
                                <span class="help-block"></span>
                            </div>
                        </div>

                        <div class="form-group">
                            <label class="control-label col-md-3">Site Invoice NO</label>
                            <div class="col-md-9">
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Invoiced Amount</label>
                            <div class="col-md-9">
                                 <select name="amt" class="form-control">
                                 <?php foreach ($invoices as $key => $value ) {?>
                                    <option value="<?php echo $value->gross;?>"><?php echo $value->invoiceNo.'  --  '.$value->gross.'--';?></option>
                                 <?php } ?>
                             </select>
                                <span class="help-block"></span>
                            </div>
